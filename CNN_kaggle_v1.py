...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 128, 5, padding=0) #was 5 96
        self.conv2 = nn.Conv2d(128, 256, 5, padding=0)  #44
        self.conv3 = nn.Conv2d(256, 256, 5, padding=0) #22
        self.conv4 = nn.Conv2d(256, 128, 5, padding=0)#9
    ...
    def forward(self, x):
        x = self.drop(self.pool(self.bn1(F.relu(self.conv1(x))))) #48
        x = self.drop(self.pool(self.bn2(F.relu(self.conv2(x))))) #22
        x = self.drop(self.pool(self.bn3(F.relu(self.conv3(x))))) #9
...
def train(images, labels):
    ...
    net = Net().to(device)
    criterion = nn.CrossEntropyLoss(weight=weight(define_classes(labels)[1][:9500]).to(
        device))
    optimizer = optim.SGD(net.parameters(), lr=0.025, weight_decay=0.015,
        momentum=0.9) #wc was 0.015 lr 0.01
    ...
    for epoch in range(500):  # loop over the dataset multiple times
        if epoch>20:
            optimizer = optim.SGD(net.parameters(), lr=0.001,
                weight_decay=0.015, momentum=0.9) #lr 0.001
        if epoch>30: #was30
            optimizer = optim.SGD(net.parameters(), lr=0.001,
                weight_decay=0.015, momentum=0.9) 
        if epoch>40: #was40
...
if __name__ == '__main__':
    ...
    images_test_dataset = kaggle_dataset(images_test, torch.tensor([0.]*len(images_test)),
        transforms=transform)
    test_loader = torch.utils.data.DataLoader(images_test_dataset, batch_size=128,
        shuffle=False, num_workers=0)
    t1 = time.time()
    ...
    plt.figure()
    plt.plot(range(500),loss_train, 'sk-', label='Train')
    plt.plot(range(500),loss_test, 'sr-', label='Valid')
    plt.xlabel('Epoch')
    plt.ylabel('Average cost')
    plt.legend()

    plt.figure()
    plt.plot(range(500),e_train, 'sk-', label='Train')
    plt.plot(range(500),e_test, 'sr-', label='Valid')
    plt.xlabel('Epoch')
    plt.ylabel('Error')
    plt.legend()
